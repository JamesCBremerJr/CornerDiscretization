        ...
        dimension a(10 000 000),b(10 000 000),a2(10 000 000)
c 
        call prini(6,13)
C 
...
c 
        ...
        implicit double precision (a-h,o-z)
        save
        double precision a(n,n),b(n,n),y2(10 000),w(10 000),
     1      x(10 000),y(10 000),x2(10 000),a2(1),diffs(10 000),
     2      xs(10 000),xs7(10 000),y7(10 000)
c 
c 
...
c 
 ...
 1400 continue
        ...
        call prin2('test matrix as created *',a,n*n)
c 
...
c 
        ...
        call qrdecom(a,n,b,rcond)
  
  
        t2=clotatim()
  
        call prin2('cpu time for qrdecom=*',t2-t1,1)
        call prin2('after qrdecom, rcond=*',rcond,1)
...
c 
 ...
 2200 continue
        ...
        call qrsolve(n,b,y,x2)
  
  
        call prin2('after qrsolve, x2=*',x2,n)
...
c 
 ...
 2400 continue
        ...
        call prin2('and x2-x=*',diffs,n)
c 
c       now, solve the linear system using qrsolv
c 
c 
c       test the solver for the adjoint system
c 
c 
c        . . . construct a test vector
c 
        do 3200 i=1,n
c 
        xs7(i)=i
 3200 continue
        ...
        call prin2('xs7 as created*',xs7,n)
c 
c        apply the putative (a^*)^{-1} to xs7
c 
        call qrsolve_adj(n,b,xs7,x)
  
        call prin2('after qrsolve_adj, x=*',x,n)
c 
c 
c        apply A^* to x
c 
        call qrmatavec(a,n,x,y7)
  
        call prin2('and y7=*',y7,n)
c 
c        estimate the condotion number of the matrix a
c 
        call qrcond(n,a,b,rcond,w)
c 
        call prin2('after qrcond, rcond=*',rcond,1)
...
c 
c 
c        This file contains six user-callable subroutines: qrsolv,
...
c 
c   qrsolve_c  - uses a version of QR-decomposition to solve the
c       user-supplied system of linear algebraic equations
c 
c              A(x)=y                                                 (2)
c 
c       with a double precision MATRIX AND A COMPLEX *8 RIGHT-HAND SIDE.
C       Hopefully, the QR-decomposition has been prepared by a
c       prior call to the subroutine qrdecom (see); this subroutine
c       has no known uses as a stand-alone device.
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        save
        double precision a(n,n),u(2,2),aa(2),rhs(n)
c 
...
c       2*n**2+n+10 double precision locations long
c  rcond - a fairly crude estimate of the condition number of a
c 
c        . . . allocate memory
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        save
        double precision a(n,n),b(n,n),w(1)
c 
...
c                      Work arrays:
c 
c  w - must be at least 2*n+2 double precision locations in length
c 
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        save
        double precision a(n,n),x(1),y(1)
c 
c       start the process
c 
        x(1)=y(1)/a(1,1)
...
c 
        ...
        implicit double precision (a-h,o-z)
        save
        double precision a(n,n),u(2,2),aa(2),b(n,n)
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        save
        double precision a(2,2),x(1),y(1)
c 
        do 1200 i=n0,n
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        save
        double precision a(n,n),x(1),y(1),cd
c 
...
c 
c 
        subroutine qrsolve_c(n,b,y,x)
        implicit double precision (a-h,o-z)
        save
        double precision b(1)
        double complex x(1),y(1)
c 
c       This subroutine uses a version of QR-decomposition to solve
c       the user-supplied system of linear algebraic equations with
c       double precision MATRIX AND double complex RIGHT-HAND SIDE. The 
c       QR-decomposition is prepared by a prior call to the subroutine
c       qrdecom (see); this subroutine has no known uses as a stand-alone
c       device.
c 
...
c             right-hand side
c 
        call qrtrinv_c(b(ia),y,b(iz),n)
        call qrmatvec_c(b(ib),n,b(iz),x)
c 
        return
        end
c 
...
c 
c 
        subroutine qrmatvec_c(a,n,x,y)
        implicit double precision (a-h,o-z)
        save
        double precision a(n,n)
        double complex x(1),y(1),cd
c 
c        apply the matrix a to the vector x obtaining y
c 
        do 1400 i=1,n
...
c 
c 
        subroutine qrtrinv_c(a,y,x,n)
        implicit double precision (a-h,o-z)
        save
        double precision a(n,n)
        double complex x(1),y(1),d
c 
c       start the process
c 
        x(1)=y(1)/a(1,1)
