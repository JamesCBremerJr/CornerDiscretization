        ...
        double precision, allocatable :: eye(:,:),pols(:),ders(:)
        double precision, allocatable :: whtslog(:),vals(:),coefs(:)


        double precision, allocatable :: ys(:),ywhts(:),amatrin(:,:)
        double precision, allocatable :: zs(:),vs(:),vals2(:)
c        
        external fun
c
...
c
        m  = 40 
        n  = 30 
        allocate(xs(n),whts(n),u(n,n),v(n,n),eye(n,n))
        allocate(pols(n),ders(n),whtslog(n),coefs(n),vals(n))
c
        allocate(ys(m),ywhts(m),amatrin(m,n),vals2(m))
c
        mm = 1 000 000
        allocate(zs(mm),vs(mm))
...
c       point, holds whenever f is a polynomial of degree n.
c
...
c
        do 1000 j=2,n
        vals(j+1)=((2.0d0*j-1.0d0)*x*vals(j)-(j-1.0d0)*vals(j-1.0d0))/j
 1000 continue
c
        end
